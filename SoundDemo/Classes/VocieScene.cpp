...
void VocieScene::btnClickCallback(cocos2d::Ref* pSender)
{
    ...
    case TESTEXIT_BTN_TAG:
        {
		log("testmsg === %s", testmsg.c_str());  // 服务器接收到的数据
		string recodByte = base64_decode(testmsg.c_str());
		log("recodByte === %s", recodByte.c_str());
		 string path = VoiceMgr::getInstance()->_voiceMediaPath + "recordttt.amr";
		 FileUtils::getInstance()->writeStringToFile(recodByte, path);
		 VoiceMgr::getInstance()->play("recordttt.amr");
		 /***********************************************************************  end **/

		 log("== test arm is exit ==");
		 string msg = VoiceMgr::getInstance()->getVoiceData("recordttt.amr");   //  得到录音数据
        }
            break;
	default:
		break;
